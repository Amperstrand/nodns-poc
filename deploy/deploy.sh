...
if [ "$PUSH" = true ]; then
    ...
    ssh "$VPS_HOST" "systemctl stop nodns-bot"
    echo "  Uploading binary to VPS..."
    scp "$BINARY" "${VPS_HOST}:${REMOTE_PATH}/nodns-bot.new"
    echo "  Swapping binary..."
    ssh "$VPS_HOST" "mv ${REMOTE_PATH}/nodns-bot.new ${REMOTE_PATH}/nodns-bot && chmod +x ${REMOTE_PATH}/nodns-bot"
    echo "  Starting service..."
    ssh "$VPS_HOST" "systemctl start nodns-bot && sleep 2 && systemctl status nodns-bot --no-pager"
    echo ""
