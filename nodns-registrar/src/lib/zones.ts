...
function parseStatusTag(
  tags: string[][],
): { status: DiscoveredZone["status"]; reason?: string } {
  const tag = tags.find((t) => t[0] === "status");
...
export async function discoverZones(): Promise<DiscoveredZone[]> {
